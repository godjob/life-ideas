...
tags: [プロンプト最適化, トークン削減, コスト削減, API利用効率, Cavemanテクニック]
---
...
[段階的AIモデル活用の3フェーズ設計](tiered-ai-model-planning-verification-execution.md)に基づく実装パターン：

```
【非効率】
全ての工程でClaude Opusを使用
→ トークン消費：月5,000ドル以上

【段階的モデル選択】
1. 計画フェーズ：Claude Haikuで要件整理と計画立案
   ("ユーザーの期待する結果をリスト化してください" 形式)
2. 検証フェーズ：Claude Sonnetで計画の妥当性確認
   ("この計画の実現可能性を評価してください" 形式)
3. 実行フェーズ：Claude Opusで複雑な分析のみ実施
   ("以下の複雑な問題について詳細に分析してください" 形式)

→ トークン消費：月2,000ドル程度（60%削減）
+ APIレート制限への到達が3週間以上減少
```
